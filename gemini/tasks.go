...
func GuesssKanji(req string) (string, error) {
	...
	resp, err := useGemini("guesssKanji", req, true)
	if err != nil {
		return "", err
	}
...
func TranslateEnUk(req string) (string, error) {
	...
	resp, err := useGemini("translateEnUk", req, false)
	if err != nil {
		return "", err
	}
...
func TranslateJaUk(req string) (string, error) {
	...
	// instruction = "Ти — ШІ-словник. Твоє звдання — допомогти перекласти/зрозуміти японський текст/вираз. (Якщо є якась помилка — вкажи на неї, запропонуй правильний варіант.) Транслітерація канджі, якщо потрібна — лише каною. Результат подати як HTML article (лише корисна інформація — жодного зайвого тексту)."

	// resp, err := useGemini(instruction, req, false)
	resp, err := useGemini("translateJaUk", req, false)
	if err != nil {
		return "", err
	}
